...
from VQABBoxpro.datasets.datasets.vqa_datasets import OKVQAEvalData,VizWizEvalData,IconQAEvalData,GQAEvalData,VSREvalData,HMEvalData
from VQABBoxpro.common.vqa_tools.VQA.PythonHelperTools.vqaTools.vqa import VQA
from VQABBoxpro.common.vqa_tools.VQA.PythonEvaluationTools.vqaEvaluation.vqaEval import VQAEval

from VQABBoxpro.common.eval_utils import prepare_texts, init_model, eval_parser
from VQABBoxpro.conversation.conversation import CONV_VISION_VQABBoxprov2
...
conv_temp = CONV_VISION_VQABBoxpro.copy()
conv_temp.system = ""
model.eval()
...
if 'okvqa' in args.dataset:
    ...
    for images, questions, question_ids, img_ids in eval_dataloader:
        ...
        for answer, question_id, question, img_id in zip(answers, question_ids, questions, img_ids):
            ...
            VQABBoxpro_predict.append(result)

    file_save_path= os.path.join(save_path,"okvqa.json")
    with open(file_save_path,'w') as f:
        json.dump(VQABBoxpro_predict, f)
